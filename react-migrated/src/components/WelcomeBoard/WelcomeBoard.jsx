...
export const WelcomeBoard = () => {
    ...
    useEffect(() => {
        const fetchDoctors = async () => {
            try {
                const response = await fetch('https://dashdev2.netlify.app/.netlify/functions/api/doctors');
                ...
                // Filter out empty name entries and sort by name
                allDoctors = allDoctors
                    .filter(d => d && d.name && d.name.trim() !== '.' && d.name.trim() !== '')
                    .map(d => ({
                        ...d,
                        name: d.name.trim(),
                        specialty: d.specialty || 'Dokter Spesialis',
                        image_url: d.image_url || 'https://placehold.co/200x200/e2e8f0/475569?text=No+Photo'
                    }))
                ...
                console.error('[WelcomeBoard] Fetch error:', err);
