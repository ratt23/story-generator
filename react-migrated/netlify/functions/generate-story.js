...
// === KONFIGURASI DASAR ===
const GOOGLE_SCRIPT_JADWAL_URL = 'https://dashdev2.netlify.app/.netlify/functions/api/doctors';
const GOOGLE_SCRIPT_CUTI_URL = 'https://dashdev2.netlify.app/.netlify/functions/api/leave';
...
function fetchData(url, redirectCount = 0) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(30000, () => {
      req.destroy();
      reject(new Error(`Timeout untuk ${url}`));
    });
  });
}
...
async function getCombinedDoctorData() {
  ...
  try {
    ...
    const doctorList = Array.isArray(jadwalData)
      ? jadwalData
      : (jadwalData.doctors && Array.isArray(jadwalData.doctors))
        ? jadwalData.doctors
        : Object.values(jadwalData).flatMap(group => (group && Array.isArray(group.doctors)) ? group.doctors.map(d => ({ ...d, specialty: group.title })) : []);
    ...
    doctorList.forEach(doc => {
      ...
      if (doc.id) doctorById.set(doc.id, doc);
      if (docName) doctorByName.set(normalizeName(docName), doc);
    });

    const leaveList = Array.isArray(cutiData)
      ? cutiData
      : (cutiData.leaves && Array.isArray(cutiData.leaves))
        ? cutiData.leaves
        : (cutiData.data && Array.isArray(cutiData.data))
          ? cutiData.data
          : [];
    ...
    const combinedData = leaveList
      .map((cuti, index) => {
        ...
        if (cuti.doctor_id && doctorById.has(cuti.doctor_id)) {
          ...
          const norm = normalizeName(doctorNameRaw);
          if (doctorByName.has(norm)) matchedDoc = doctorByName.get(norm);
        }
